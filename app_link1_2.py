    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            if 'prompt_idx' in df.columns:
                df = df[(df['prompt_idx'] >= 101) & (df['prompt_idx'] <= 220)].copy()
            else:
                try:
        ...
        with main_col2:
            ...
            if question_list:
                for q_text in question_list:
                    is_checked = q_text in previous_score['checked_features']
                    checked_state[q_text] = st.checkbox(q_text, value=is_checked)
            else:
                st.info("无特定特征")
...
def main():
    ...
    st.sidebar.header("👤 Link 1.2 (101-220)")
